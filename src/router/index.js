...
import { setToken,getToken } from '@/utils/tools.js';
 ...
 */
export const canTurnTo = (name, access, routes) => {  //这一段代码是错误的
...
router.beforeEach((to,from,next)=>{
  const token = getToken(); //从cookie中拿到token
  if(!token && to.name !== 'login'){ //未登录且要跳转的页面不是登录页
    ...
    if(store.state.user.access){ //有权限
      ...
      store.dispatch('getUserInfo').then(user=>{
        ...
        //如：['s uper_admin'] ['super_admin', 'admin']
        //turnTo(to,user.data.data,next);
        to = {name:"home"}
        refresh(to,store.state.user.access,next,from); //刷新权限
      }).catch(()=>{
        setToken('');
        next({name:'login'});
      });
